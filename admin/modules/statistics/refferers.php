...
//---------------------------------------------------------------------------------------------------
if($q = mysql_query("SELECT s.from_domain_id, d.domain, COUNT(DISTINCT s.visitor_id) AS visitors, COUNT(*) AS visits, 
...
WHERE s.to_domain_id = '".$domains['domain_id']."' 
	AND s.from_domain_id NOT IN (0,".$domains['domain_id'].") 
	AND s.date_time BETWEEN '".$min."' AND '".$max."'
GROUP BY s.from_domain_id ORDER BY visitors DESC, visits DESC")){	
	while($r = mysql_fetch_assoc($q)){
		
		$str .= '<tr>
			<td><p><input type="text" value="'.htmlspecialchars($r['domain']).'" readonly></p></td>
			<td><p>'.$r['visitors'].'</p></td>
			<td><p>'.$r['visits'].'</p></td>
			<td><p>'.$r['from_paths'].'</p></td>
			<td><p>'.$r['to_paths'].'</p></td>
			<td><p><a href="'.$_SERVER['SYSTEM']['URL_MODULE_DIR'].(($r['visitors']=='1')?'visitor.php?visitor_id='.$r['visitor_id'] : 'visitors.php?refferer_id='.$r['from_domain_id']).$date.'" target="_blank">Подробнее»</a></p></td>
		</tr>';
...
echo $_SERVER['SITE_HEADER'].Head('Популярность реффереров','','<td style="white-space:nowrap">

<?php
/** $conn->processUntil('session_start'); дальше в зацикливании private function __process($maximum=5) {
...
/** XMPPHP_Exception */
require_once dirname(str_replace('\\','/',__FILE__)) . '/Exception.php';

/** XMPPHP_XMLObj */
require_once dirname(str_replace('\\','/',__FILE__)) . '/XMLObj.php';

/** XMPPHP_Log */
require_once dirname(str_replace('\\','/',__FILE__)) . '/Log.php';
...
class XMPPHP_XMLStream {
	...
	private function __process($maximum=5) {
		...
		do {
			...
			log_('+8.2.15=='.$endtime.'-'.$time_past.'='.$remaining.'**$maximum='.$maximum.'=is_null:='.is_null($maximum).'=>='.($remaining > 0));
